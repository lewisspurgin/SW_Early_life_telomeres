...
x <- summary(lm(TLKB~Insect,data= FlSAall))$resid
xf <- ifelse(x>median(x),'Long','Short')

s1 <- survfit(Surv(time = Lifespan+1, event = Died)~xf, data = FlSAall)
Fig2 <- ggsurv(s1,plot.cens=F) + 
  theme_classic() +
  ...
  scale_colour_manual(name = 'Residual TL',
                      values = c('darkred','red')) +
  guides(linetype=F) +
  theme(legend.title = element_text('Telomere length'))
