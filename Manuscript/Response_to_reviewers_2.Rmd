...
Thus if within-group variation is extremely small (i.e. no measurement error), the ICC would be 1, so 0% of the variation would be explained by measurement error. Thus we would interpret an ICC of 0.68 as 68% of the variation being explained by sample identity and 32% by variation other than sample identity (i.e. measurement error).

However, this feels like a debate over semantics. In the manuscript, we are careful not to phrase things in terms of repeatability, and nowhere do we state that 68% of the variance can be explained by sample identity.
...
RESPONSE: We are glad that the reviewer agrees that our observed repeatability/efficiency values will not lead to false positives. We agree that this issue deserves more discussion space, and have now addressed the issue of repeatability, and the specific issue of qPCR efficiency, in the discussion on lines XX:
