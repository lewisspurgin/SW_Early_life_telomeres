...
For a large subset of birds we had longitudinal data, with two or more samples taken at different ages (n = `r length(unique(c(dd3$BloodID,dd3$BloodID1)))` measurements from `r length(unique(dd3$BirdID))` birds). For these individuals we calculated the within-individual change in RTL by subtracting RTL at time point *t* from RTL at time point *t* + 1 (hereafter $\Delta$RTL, n = `r nrow(dd3)` measurements). Negative values of RTL reflect decreases in telomere length with age, while positive values reflect increases. Individuals were not always caught at the same time of year, but within a 3 month breeding season window.
