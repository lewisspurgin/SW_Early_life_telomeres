...
For a large subset of birds we had longitudinal data, with two or more samples taken at different ages (n = `r length(unique(c(dd3$BloodID,dd3$BloodID1)))` measurements from `r length(unique(dd3$BirdID))` birds). For these individuals we calculated the within-individual change in RTL by subtracting RTL at time point *t* from RTL at time point *t* + 1 (hereafter $\Delta$RTL, n = `r nrow(dd3)` measurements). Negative values of RTL reflect decreases in telomere length with age, while positive values reflect increases. Note that the follow-up time between samples varied among individuals, and that individuals are not always caught at exactly the same time of year.
