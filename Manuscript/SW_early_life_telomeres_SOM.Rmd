...
#Run the model
lm1 <- lmer(RTL ~ Agemonths * (Tarsus + Helper + NonHelper + Sex + BodyMass  + TQ) + (1|LayYear)+(1|TerritoryID),
            data = juv,
            REML = FALSE)
lm1 <- standardize(lm1)
...
#Run the model
lm1 <- lmer(TROC ~ Agemonths * (Tarsus + Helper + NonHelper + Sex + BodyMass  + TQ) + RTL + (1|LayYear),
            data = temp,
            REML = FALSE)
lm1 <- standardize(lm1)
...
tableS2 <- data.frame(Model = apply(t1,1,getterms,c(2:15)),t1[,c('df','AICc','delta','weight')],stringsAsFactors = FALSE)
...
#####################
temp <- subset(Loss,TimeDiff>0.3)
temp <- subset(temp,Agemonths == 1)
temp <- subset(temp,TROC<2)
temp <- subset(temp,TQI<4)
...
ggplot(temp,aes(x = RTL,y = TROC,col=factor(Helper)))+
geom_point()+
