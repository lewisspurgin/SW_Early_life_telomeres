...
#Run the model
lm1 <- lmer(RTL ~ Agemonths * (Tarsus + Helper + NonHelper + Sex + BodyMass  + TQ + Season) + (1|LayYear)+(1|TerritoryID),
            data = juv,
            REML = FALSE)
lm1 <- standardize(lm1)
...
tableS1 <- data.frame(Model = apply(t1,1,getterms,c(2:16)),t1[,c('df','AICc','delta','weight')],stringsAsFactors = FALSE)
...
#Run the model
lm1 <- lmer(TROC ~ Agemonths * (Tarsus + Helper + NonHelper + Sex + BodyMass  + TQ + Season) + (1|LayYear),
            data = temp,
            REML = FALSE)
lm1 <- standardize(lm1)
...
tableS2 <- data.frame(Model = apply(t1,1,getterms,c(2:16)),t1[,c('df','AICc','delta','weight')],stringsAsFactors = FALSE)
