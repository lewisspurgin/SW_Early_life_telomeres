...
#Run the model
lm1 <- lmer(Loss ~ (Tarsus + Helper + NonHelper + Sex + BodyMass  + TQ + Season) + (1|LayYear),
            data = temp,
            REML = FALSE)
lm1 <- standardize(lm1)
...
tableS2 <- data.frame(Model = apply(t1,1,getterms,c(2:8)),t1[,c('df','AICc','delta','weight')],stringsAsFactors = FALSE)
