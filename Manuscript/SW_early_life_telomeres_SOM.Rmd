...
#Run the model
lm1 <- glmer(L ~ Agemonths*(Tarsus + Helper + NonHelper + Sex + BodyMass  + TQ) + (1|FieldPeriodID) + (1|TerritoryID),
            data = subset(temp,Loss<0),
            REML = FALSE)
lm1 <- standardize(lm1)
...
ggplot(d5,aes(x = RTL,col = factor(LayYear)))+
geom_density()

with(juv,tapply(RTL,LayYear,length))

d5 <- subset(juv,LayYear == 2002)
plot(RTL~BodyMass,data=d5)

lm1 <- lmer(RTL ~ Agemonths*(Tarsus + Helper + NonHelper + Sex + BodyMass  + TQ) + (1|TerritoryID),
            data = d5,
            REML = FALSE)
lm1 <- standardize(lm1)
