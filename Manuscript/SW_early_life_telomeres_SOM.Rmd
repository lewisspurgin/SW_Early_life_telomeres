...
#Run the model
lm1 <- lmer(cenTL ~ Agemonths * (Tarsus + Helper + NonHelper + Sex + BodyMass  + TQ) + (1|FieldPeriodID)+(1|TerritoryID)+(1|PlateID),
            data = juv,
            REML = FALSE)
lm1 <- standardize(lm1)
...
#Run the model
lm1 <- lmer(Loss ~ Agemonths*(Tarsus + Helper + NonHelper + Sex + BodyMass  + TQ) + (1|FieldPeriodID) + (1|TerritoryID),
            data = temp,
            REML = FALSE)
lm1 <- standardize(lm1)
