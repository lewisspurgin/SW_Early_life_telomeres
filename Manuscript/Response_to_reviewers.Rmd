...
RESPONSE: We thank the editor and reviewers for their positive and constructive comments. We have now addressed the methodological questions raised by reviewers one and three, and have updated several of our analyses in line with their suggestions. We have also performed additional analyses to show that sample storage time does not affect telomere length, and included a discussion on repeatability. While almost none of our results have changed, we believe that incorporating the reviewer comments has substantially improved the clarity, readability and rigour of our work. Finally, we have updated our bibliography, in accordance with the reviewer comments.
...
RESPONSE: Interestingly, this is the one effect that has changed from  including a plate ID random effect in our analyses, with delta AIC now ~11. However, we do not wish to place any strong conclusions on this analysis. We agree that while there is convincing evidence for cohort effects on telomere length, the evidence for cohort effects on telomere dynamics is weaker. We have therefore removed references to this effect throughout the discussion, and allow readers to interpret the effect as they wish.
...
RESPONSE: We are not sure that "worrying" is the correct phrase to use, as we anticipate that other studies in natural population will find similar patterns. Further, we disagree with the sentiment that a biomarker must be consistent over individual lifespans to be useful - rather, a useful marker needs to change with ecological context. We feel that we cover this point in the discussion, lines XX:
...
However, as we argue above, we do not think it is likely that the pattern is a result of sample storage/DNA integrity. We have no evidence from our samples that there is sample degradiation with respect to DNA integrity or telomere length (see above) - something confirmed by other studies (see Reichert et al. 2017; Oecologia)
...
RESPONSE: We now clarify (lines XX) that the territory quality estimates that we use in our statistical models are taken from the field period in which the telomere sample was taken. The same applies to insect abundance, group size etc. We feel that an in-depth explanation of how and why territory quality varies over space and time is outside the scope of this manuscript, especially given i) the word limit, and ii) the fact that territory quality is not related to RTL. However we do now include a reference to recent book chapter (Komdeur et al. 2016) which explains how and why territory quality varies on Cousin Island. Our updated section on territory quality and indect abundance is now on lines XX:
...
\textit{"Individuals were not always caught at the same time of year, but within a 3 month breeding season window."}
...
RESPONSE: We now include a formal test of the within and between individual slopes, as described in van de Pol and Wright (2009), and find that there is no difference between cross-sectional and longitudinal telomere shortening - lines XX:

\begin{adjustwidth}{2cm}{2cm}

\textit{"Further, we found no evidence that within and between individual slopes of telomere shortening varied (estimate = -0.003, CIs = -0.008, 0.003; see Van de Pol and Wright 2009), suggesting that there was no difference between cross-sectional and longitudinal telomere shortening with age."}

Not also that this analysis was done in one of the original Seychelles warbler telomere studies (Barrett et al. 2013; Mol. Ecol.)
