...
Fig4B <- ggplot(temp,aes(x = Agemonths,y = CqGAPDH,group = BirdID,col=factor(BirdID)))+
geom_point()+
geom_line()+
  xlab('Age (months)') +
  theme_classic() +
...
ggplot(ddL,aes(y = DeltaTL,x = DeltaGAP, col = Group))+
geom_point()+
stat_smooth(method = 'lm')


plot(DeltaTL~Agemonths,data=dd3)
plot(DeltaGAP~Agemonths,data=dd3)


plot(DeltaGAP~TimeDiff,data=dd3)

hist(dd$CqGAPDH)

plot(CqTelomere~CqGAPDH,data=dd)

boxplot(CqTelomere~Ageclass,data=dd)
boxplot(CqGAPDH~Ageclass,data=dd)


ggplot(dd,aes(y = CqGAPDH, x = Agemonths,col=Sex))+
geom_point()+
stat_smooth(method = 'lm')

summary(lm(CqGAPDH~Agemonths,data=dd))
summary(lm(CqTelomere~Agemonths,data=dd))
summary(lm(RTL~Agemonths,data=dd))
