...
temp$InsectF <- ifelse(temp$Insect>4,'High','Low')
Fig3C <- ggplot(temp,
       aes(
           y = RTL,
           x=Agemonths,
...
anova(lm(RTL~Sex+Agemonths+factor(CatchYear)+Sex*factor(LayYear),data=dd))


ggplot(dd,aes(x = factor(LayYear),y = RTL,col = Sex))+
