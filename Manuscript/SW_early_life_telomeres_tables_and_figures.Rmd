...
temp <- Loss
#Run the model
lm2 <- lmer(Loss ~ TimeDiff+Agemonths*Helper+Tarsus + NonHelper + Sex + BodyMass  + TQ + (1|FieldPeriodID),
            data = temp,
            REML = FALSE)
lm2 <- standardize(lm2)
...
Fig5B <- ggplot(temp,
  ...
  scale_fill_manual(values = c('grey','darkorange'))+
  geom_abline(slope = 0,intercept = 0,lty = 2)+
  annotate('text',x = 0.55,y = 1.5,label = 'B')
...
Fig5C <- ggplot(temp,
  ...
  scale_colour_manual(values = c('grey','darkorange','darkgreen'))+
  scale_fill_manual(values = c('grey','darkorange','darkgreen'))+
  annotate('text',x = 3,y = 1.3,label = 'C')

pdf('Figures/Figure 5.pdf',
    width = 8,
    height = 8,
    paper = 'a4')
multiplot(Fig5A,Fig5B,Fig5C,layout = matrix(c(1,1,2,3),nrow=2,byrow = T))
dev.off()
