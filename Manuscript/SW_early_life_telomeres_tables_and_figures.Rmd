...
Fig3A <- forestplot(table2)+
  annotate('text',x = 0.8,y = 0.25,label = 'A')
...
temp <- Loss
#Run the model
lm2 <- lmer(Loss ~ Agemonths+Helper+Tarsus + NonHelper + Sex + BodyMass  + TQ + (1|FieldPeriodID),
            data = temp,
            REML = FALSE)
lm2 <- standardize(lm2)
...
table2 <- data.frame(table2,myx=row.names(table2))
table2 <- table2[rownames(table2) != 'z.TimeDiff',]
table2$myx <- factor(table2$myx,levels = table2$myx[order(table2$Estimate,decreasing = T)])

Fig4C <- forestplot(table2)+
  annotate('text',x = 0.75,y = 0.6,label = 'C')
...
pdf('Figures/Figure 4.pdf',
    ...
    height = 8,
    paper = 'a4')
multiplot(Fig4A,Fig4B,Fig4C,layout=matrix(c(1,2,3,3),nrow=2,byrow=T))
dev.off()
...
```{r Figure 5}
temp <- subset(juv13,Agemonths != 1)
temp$LayYear <- factor(temp$LayYear)
temp[,'Survived to following year?'] <- ifelse(temp$SurvivedNext == 1,'Y','N')

Fig4B <- ggplot(temp,
       aes(x = LayYear,
           y = RTL,
           fill = `Survived to following year?`)) +
  xlab('Cohort') +
  theme_classic() +
  theme(axis.text.x = element_text(size = 12), 
        axis.text.y = element_text(size = 12), 
  ...
  geom_jitter(aes(col = `Survived to following year?`),position=position_jitterdodge(jitter.width = 0.2))+
  annotate('text',x = 0.5,y = 1.5,label = 'B')
