...
**Figure 4** Longitudinal telomere dynamics in subset(dd,Agemonths<12)enile Seychelles warblers. **A** subset(dd,Agemonths<12)enile telomere length in relation to adult telomere length measured in the same individual. **B** $\Delta$RTL in relation to the time between sampling events. Lines and shaded areas represent fitted values and 95% confidence limits from general linear models. Raw data have been ommited from **B** for the sake of clarity, but are plotted in Figure S1.

**Figure 5** Factors affecting $\Delta$RTL in subset(dd,Agemonths<12)enile Seychelles warblers. **A** Model averaged estimates and 95% confidence intervals for explanatory terms used in a linear mixed model with $\Delta$RTL as the response variable (see main text for details; TQ = territory quality). **B** $\Delta$RTL in relation to tarsus length and sex.
...
Fig4A <- ggplot(temp,
       aes(x = RTL,
           ...
           col = LossF)) +
  ylab('Adult RTL') + 
  xlab('subset(dd,Agemonths<12)enile RTL') +
    xlim(c(0,3))+
  ylim(c(0,3))+
  theme_classic() +
  theme(axis.text.x = element_text(size = 12), 
...
Fig4D <- ggplot(temp,aes(col=DeltaRTLF,y = DeltaRTL,x = TimeDiff))+
    geom_point()+
  theme_classic()+ 
...
temp <- with(dd3,tapply(DeltaRTL,Agemonths,mean))
tempSE <- with(dd3,tapply(DeltaRTL,Agemonths,se))*1.96

plot(temp~names(temp),type = 'l',ylim = c(-0.8,0.8))
points(temp+tempSE~names(temp),type = 'l',lty = 2)
points(temp-tempSE~names(temp),type = 'l',lty = 2)
abline(0,0,col = 'red',lwd = 2)


plot(DeltaRTL~TimeDiff,data = dd3)
