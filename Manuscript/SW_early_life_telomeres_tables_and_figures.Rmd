...
#Run the model
lm2 <- lmer(DeltaRTL ~  LogAge + Tarsus + Helper + GroupSize + Sex + BodyMass + Insect + Density + (1|LayYear) + (1|BirdID) + (1|PlateID),
            data = temp,
            REML = FALSE)
CIs <- confint.merMod(lm2)
t1 <- data.frame(Estimate = fixef(lm2), s.e. = se.fixef(lm2), LCI = CIs[5:nrow(CIs),1], UCI = CIs[5:nrow(CIs),2])
...
Fig3A <- transform(juv,Cohort = factor(CatchYear)) %>%
...
Fig3B <- transform(dd,Cohort = factor(CatchYear)) %>%
  ddply(.(Cohort),
        mutate,
