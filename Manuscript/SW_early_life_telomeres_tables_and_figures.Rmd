...
Fig1B <- ggplot(temp,aes(x = `Age (months)`,y = TROC)) +
    geom_jitter(col='grey') +
  ylab('Loss in RTL') + 
  xlab('Age (months)') +
  theme_classic() +
...
Fig1D <- ggplot(temp,
  ...
  geom_boxplot(outlier.colour = NA)+
  scale_x_discrete(limits = factor(1998:2014),drop=F)
  annotate('text',x = 1,y = 4.4,label = 'D')
...
pdf('Figures/Figure 1.pdf',
    ...
    height = 8,
    paper = 'a4r')
multiplot(Fig1A,Fig1B,Fig1C,Fig1D,layout=matrix(c(1,2,3,3,4,4),nrow = 3,byrow=T))
dev.off()
...
#Run the model
lm2 <- lmer(RTL ~ Agemonths * (Tarsus + Helper + NonHelper + Sex + BodyMass  + TQ)  + (1|LayYear) + (1|TerritoryID),
            data = temp,
            REML = FALSE)
lm2 <- standardize(lm2)
#Model averaging
...
pdf('Figures/Figure 2.pdf',
    ...
    height = 9,
    paper = 'a4r')
multiplot(Fig2A,Fig2B,Fig2C,Fig2D,layout = matrix(c(1,1,1,2,3,4),nrow = 2,byrow = T))
dev.off()
...
#Run the model
lm2 <- lmer(TROC ~ Agemonths * (Tarsus + Helper + NonHelper + Sex + BodyMass  + TQ)  + RTL + (1|LayYear),
            data = temp,
            REML = FALSE)
lm2 <- standardize(lm2)
#Model averaging
...
Fig4A <- ggsurv(s1,plot.cens=F,lty.est=c(1:2)) + 
  theme_classic() +
