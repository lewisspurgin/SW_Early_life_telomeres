...
nams[nams == 'Agemonths'] <- 'Age (continuous)'
nams[nams == 'LogAge'] <- 'Log age'
nams[nams == 'FAge'] <- 'Age (factor)'
nams[nams == 'Cohort:FAge'] <- 'Age (factor)*cohort'
nams[nams == 'Cohort:Agemonths'] <- 'Age (continuous)*cohort'
nams[nams == 'Cohort:LogAge'] <- 'Log age*cohort'

colnames(t1) <- nams

tableS1 <- data.frame(Model = apply(t1,1,getterms,c(2:8)),t1[,c('df','AICc','delta','weight')],stringsAsFactors = FALSE)
