...
tableS1A <- data.frame(Model = apply(t1,1,getterms,c(2:10)),t1[,c('df','AICc','delta','weight')],stringsAsFactors = FALSE)
colnames(tableS1A) <- c('Model','df','AICc', 'Delta AICc','Weight')
rownames(tableS1A) <- NULL
tableS1A$Model[tableS1A$Model == ''] <- 'Null model'
tableS1A[,3:5] <- lapply(tableS1A[,3:5],as.numeric)
...
tableS1B <- data.frame(Model = apply(t1,1,getterms,c(2:8)),t1[,c('df','AICc','delta','weight')],stringsAsFactors = FALSE)
colnames(tableS1B) <- c('Model','df','AICc', 'Delta AICc','Weight')
rownames(tableS1B) <- NULL
tableS1B$Model[tableS1B$Model == ''] <- 'Null model'
tableS1B[,3:5] <- lapply(tableS1B[,3:5],as.numeric)

tableS1 <- rbind(c('A',rep(' ',4)),tableS1A,c('--',rep(' ',4)),c('B',rep(' ',4)),tableS1B)
...
CIs <- CIwald(RTL_Full)
t1a <- data.frame(Variable = row.names(CIs)[7:nrow(CIs)],
                  Estimate = fixef(RTL_Full),
                  LCI = CIs[7:nrow(CIs),1],
                  UCI = CIs[7:nrow(CIs),2],
                  row.names = NULL,
                  stringsAsFactors = F)
...
t1b <- data.frame(Variable = row.names(CIs)[4:nrow(CIs)],
                  Estimate = fixef(DeltaRTL_Full),
                  LCI = CIs[4:nrow(CIs),1],
                  UCI = CIs[4:nrow(CIs),2],
                  row.names = NULL,
                  stringsAsFactors = F)
...
tableS2 <- rbind(c('A',rep(' ',4)),t1a,c('--',rep(' ',4)),c('B',rep(' ',4)),t1b)
